    ...
    def _make_request(self, method: str, path: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                raise SecretAlreadyExistsException(f"Secret already exists at path: {path}")
            ...
            for key in keys:
                ...
                try:
                    ...
                    if self.kv_version == '2':
                        ...
                        current_version = meta_response.get('data', {}).get('current_version')
                        ...
                        secrets.append(SecretMetadata(
                            ...
                            path=full_path,
                            is_kv=True,
                            version=str(current_version) if current_version else '1',
                            created_at=created_at,
                            updated_at=created_at,
                    ...
                    logger.warning(f"Failed to get metadata for secret '{full_path}': {str(e)}")
                    continue
    ...
    def create_secret(self, path: str, value: str, metadata: Optional[Dict[str, Any]] = None) -> SecretMetadata:
        ...
        try:
            ...
            if self.kv_version == '2':
                ...
                payload['options']['cas'] = 0  # cas=0 means only create if doesn't exist
            else:
                # KV v1
                payload = data
            ...
            if self.kv_version == '2':
                ...
                if version_data.get('created_time'):
                    try:
                        created_at = datetime.fromisoformat(version_data['created_time'].replace('Z', '+00:00'))
                    except Exception:
                        pass
    ...
    def update_secret(self, path: str, value: str) -> SecretMetadata:
        ...
        try:
            ...
            if self.kv_version == '2':
                ...
                if version_data.get('created_time'):
                    try:
                        updated_at = datetime.fromisoformat(version_data['created_time'].replace('Z', '+00:00'))
                    except Exception:
                        pass
    ...
    def get_secret_versions(self, path: str) -> List[str]:
        ...
        try:
            ...
            for version_num, version_info in versions_data.items():
                if not version_info.get('destroyed', False) and not version_info.get('deletion_time'):
