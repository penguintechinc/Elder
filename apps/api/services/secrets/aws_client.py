...
class AWSSecretsManagerClient(SecretProviderClient):
    ...
    def _init_client(self) -> None:
        ...
        try:
            ...
            logger.info(f"Initialized AWS Secrets Manager client for region {self.config['region']}")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(f"AWS Secrets Manager connection test failed: {e.response['Error']['Message']}")
            return False
        except Exception as e:
    ...
    def get_secret(self, path: str, version: Optional[str] = None) -> SecretValue:
        ...
        try:
            ...
            return SecretValue(
                ...
                updated_at=response.get('CreatedDate'),  # AWS uses CreatedDate for last update
                metadata={
                    'arn': response['ARN'],
            ...
            if error_code == 'ResourceNotFoundException':
                ...
                raise SecretAccessDeniedException(f"Access denied to secret '{path}': {error_msg}")
            else:
                raise SecretProviderException(f"AWS error retrieving secret '{path}': {error_msg}")
    ...
    def create_secret(self, path: str, value: str, metadata: Optional[Dict[str, Any]] = None) -> SecretMetadata:
        ...
        try:
            ...
            if error_code == 'ResourceExistsException':
                raise SecretAlreadyExistsException(f"Secret '{path}' already exists: {error_msg}")
            elif error_code == 'AccessDeniedException':
                raise SecretAccessDeniedException(f"Access denied creating secret '{path}': {error_msg}")
            else:
                raise SecretProviderException(f"AWS error creating secret '{path}': {error_msg}")
    ...
    def update_secret(self, path: str, value: str) -> SecretMetadata:
        ...
        try:
            ...
            if error_code == 'ResourceNotFoundException':
                ...
                raise SecretAccessDeniedException(f"Access denied updating secret '{path}': {error_msg}")
            else:
                raise SecretProviderException(f"AWS error updating secret '{path}': {error_msg}")
    ...
    def delete_secret(self, path: str, force: bool = False) -> bool:
        ...
        try:
            ...
            if error_code == 'ResourceNotFoundException':
                ...
                raise SecretAccessDeniedException(f"Access denied deleting secret '{path}': {error_msg}")
            else:
                raise SecretProviderException(f"AWS error deleting secret '{path}': {error_msg}")
    ...
    def get_secret_versions(self, path: str) -> List[str]:
        ...
        try:
            ...
            if error_code == 'ResourceNotFoundException':
                ...
                raise SecretAccessDeniedException(f"Access denied accessing secret '{path}': {error_msg}")
            else:
                raise SecretProviderException(f"AWS error listing versions for '{path}': {error_msg}")

        except Exception as e:
            raise SecretProviderException(f"Unexpected error listing versions for '{path}': {str(e)}")
