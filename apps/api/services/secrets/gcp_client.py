...
class GCPSecretManagerClient(SecretProviderClient):
    ...
    def _validate_config(self) -> None:
        ...
        if 'project_id' not in self.config:
            raise InvalidSecretConfigException("Missing required GCP config field: project_id")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(f"GCP Secret Manager connection test failed: Permission denied - {str(e)}")
            return False
        except Exception as e:
    ...
    def get_secret(self, path: str, version: Optional[str] = None) -> SecretValue:
        ...
        try:
            ...
            raise SecretNotFoundException(f"Secret '{path}' not found in GCP Secret Manager")
        except google_exceptions.PermissionDenied as e:
            raise SecretAccessDeniedException(f"Access denied to secret '{path}': {str(e)}")
        except Exception as e:
            raise SecretProviderException(f"GCP error retrieving secret '{path}': {str(e)}")
    ...
    def list_secrets(self, prefix: Optional[str] = None) -> List[SecretMetadata]:
        ...
        try:
            ...
            raise SecretAccessDeniedException(f"Access denied listing secrets: {str(e)}")
        except Exception as e:
    ...
    def create_secret(self, path: str, value: str, metadata: Optional[Dict[str, Any]] = None) -> SecretMetadata:
        ...
        try:
            ...
            raise SecretAlreadyExistsException(f"Secret '{path}' already exists in GCP Secret Manager")
        except google_exceptions.PermissionDenied as e:
            raise SecretAccessDeniedException(f"Access denied creating secret '{path}': {str(e)}")
        except Exception as e:
            raise SecretProviderException(f"GCP error creating secret '{path}': {str(e)}")
    ...
    def update_secret(self, path: str, value: str) -> SecretMetadata:
        ...
        try:
            ...
            raise SecretAccessDeniedException(f"Access denied updating secret '{path}': {str(e)}")
        except Exception as e:
            raise SecretProviderException(f"GCP error updating secret '{path}': {str(e)}")
    ...
    def delete_secret(self, path: str, force: bool = False) -> bool:
        ...
        try:
            ...
            raise SecretAccessDeniedException(f"Access denied deleting secret '{path}': {str(e)}")
        except Exception as e:
            raise SecretProviderException(f"GCP error deleting secret '{path}': {str(e)}")
    ...
    def get_secret_versions(self, path: str) -> List[str]:
        ...
        try:
            ...
            raise SecretAccessDeniedException(f"Access denied accessing secret '{path}': {str(e)}")
        except Exception as e:
            raise SecretProviderException(f"GCP error listing versions for '{path}': {str(e)}")
