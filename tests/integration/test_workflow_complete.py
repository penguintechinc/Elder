...
class TestCompleteWorkflow:
    ...
    def test_entity_type_distribution(self, app):
        ...
        with app.app_context():
            ...
            entities = [
                Entity(name="DC1", entity_type=EntityType.DATACENTER, organization_id=org.id),
                Entity(name="10.0.0.0/16", entity_type=EntityType.SUBNET, organization_id=org.id),
                Entity(name="server-01", entity_type=EntityType.COMPUTE, organization_id=org.id),
                Entity(name="router-01", entity_type=EntityType.NETWORK, organization_id=org.id),
                Entity(name="admin", entity_type=EntityType.USER, organization_id=org.id),
                Entity(name="CVE-2024-1234", entity_type=EntityType.SECURITY_ISSUE, organization_id=org.id),
            ]
    ...
    def test_dependency_graph_traversal(self, app):
        ...
        with app.app_context():
            ...
            entity_a = Entity(name="A", entity_type=EntityType.COMPUTE, organization_id=org.id)
            entity_b = Entity(name="B", entity_type=EntityType.COMPUTE, organization_id=org.id)
            entity_c = Entity(name="C", entity_type=EntityType.COMPUTE, organization_id=org.id)
            entity_d = Entity(name="D", entity_type=EntityType.COMPUTE, organization_id=org.id)
            db.session.add_all([entity_a, entity_b, entity_c, entity_d])
            db.session.commit()

            dep_ab = Dependency(source_entity_id=entity_a.id, target_entity_id=entity_b.id, dependency_type="depends_on")
            dep_bc = Dependency(source_entity_id=entity_b.id, target_entity_id=entity_c.id, dependency_type="depends_on")
            dep_cd = Dependency(source_entity_id=entity_c.id, target_entity_id=entity_d.id, dependency_type="depends_on")
